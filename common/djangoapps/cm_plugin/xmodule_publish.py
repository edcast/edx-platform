...
from common.djangoapps.cm_plugin.models import XModule_Metadata_Cache
...
class ProcessCacheTask(PeriodicTask):
    ...
    def create_or_update_at_cm(params):
        if params['cm_id'] is None:
            cm_id = ProcessCacheTask.create_at_cm(params)
        else:
            cm_id = ProcessCacheTask.update_at_cm(params)
        return cm_id
    ...
    def create_at_cm(params):
        post_params = ProcessCacheTask.convert_to_cm_params(params)
...
def cache_metadata(obj, obj_type, location, course, modulestore):
    ...
    from cms.djangoapps.contentstore.utils import compute_unit_state, UnitState
    params = {}
