...
#   the codejail service. Requires setting CODE_JAIL_REST_SERVICE_CLIENT_ID and
#   CODE_JAIL_REST_SERVICE_CLIENT_SECRET. If not specified, no authorization header will
#   be sent.
CODE_JAIL_REST_SERVICE_OAUTH_URL = getattr(settings, 'CODE_JAIL_REST_SERVICE_OAUTH_URL', None)
# .. setting_name: CODE_JAIL_REST_SERVICE_CLIENT_ID
# .. setting_default: None
...
CODE_JAIL_REST_SERVICE_CLIENT_ID = getattr(settings, 'CODE_JAIL_REST_SERVICE_CLIENT_ID', None)
# .. setting_name: CODE_JAIL_REST_SERVICE_CLIENT_SECRET
# .. setting_default: None
# .. setting_description: The OAuth client credential secret to use when making calls to
#   the codejail service. If not specified, no authorization header will be sent.
CODE_JAIL_REST_SERVICE_CLIENT_SECRET = getattr(settings, 'CODE_JAIL_REST_SERVICE_CLIENT_SECRET', None)
...
def _get_codejail_client():
    ...
    are configured.
    """
    if CODE_JAIL_REST_SERVICE_OAUTH_URL and CODE_JAIL_REST_SERVICE_CLIENT_ID and CODE_JAIL_REST_SERVICE_CLIENT_SECRET:
        return OAuthAPIClient(
            base_url=CODE_JAIL_REST_SERVICE_OAUTH_URL,
            client_id=CODE_JAIL_REST_SERVICE_CLIENT_ID,
            client_secret=CODE_JAIL_REST_SERVICE_CLIENT_SECRET,
        )
    else:
        return requests
